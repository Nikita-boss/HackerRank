...
string eval(string a, string b) {
    regex myreg("(.*)@gmail.com");
    if (regex_match(b, myreg)) {
        return a;
    } else {
      return {};
    }
...
int main()
{
    ...
    for (int N_itr = 0; N_itr < N; N_itr++) {
        ...
        string test = eval(firstName, emailID);
        if (test != string()) {
          mystring.push_back(test);
        }
    }

    sort(mystring.begin(), mystring.end());
    for_each(mystring.begin(), mystring.end(), [](string i){cout << i << '\n';});
